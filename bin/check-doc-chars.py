...
allowed_chars_list = ['中文', '®', '℃', '…', '✅', '❌', '√', '×', '├', '└', '│', '–', '—']
not_allowed_chars_list = re.compile(b'[^\x0a\x0d\x20-\x7e]')
file_white_list = ['index_of_abbreviations.rst']

def get_file_list(doc_path, subdir_file_list):
    ...
    for file in file_list:
        ...
        if os.path.isdir(cur_path):
            get_file_list(cur_path, subdir_file_list)
        else:
            subdir_file_list.append(cur_path)
    return subdir_file_list

def data_is_allowed_chars(match_info, data):
    ...
    for cur_idx in to_check_idx:
        if (cur_idx == s_last_idx + 1):
            ...
            chars_is_valid = 0
            for chars in allowed_chars_list:
...
def check_doc_chars_validity(doc_name):
    with open(doc_name, "rb") as fp:
        for (lineno, data) in enumerate(fp):
            match_info = re.search(not_allowed_chars_list, data)
            if match_info:
                if not data_is_allowed_chars(match_info, data):
                    print("\033[31mError: illegal character detected at %s:%d\033[0m" %(doc_name, lineno + 1))
                    print("raw data ----> %s\r\n" %data)
                    print("Allowed chars:")
                    for x in allowed_chars_list:
                        print(x, "---->", x.encode())
                    return False
            pass
    return True
...
def _main():
    ...
    en_doc_file_list = get_file_list(dst_path, [])
    for current_file in en_doc_file_list:
        for file_basename in file_white_list:
            if os.path.basename(current_file) == file_basename:
                continue
            else:
                if check_doc_chars_validity(current_file) == False:
                    sys.exit(-1)
