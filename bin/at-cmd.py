...
# AT CMD Port
# ========================================
class ATCMDPort(ESPSerialPortBase):
    def __init__(self, port: str, baudrate: int, logger: ESPLogger):
        super().__init__(port, baudrate, logger, tag="AT")
    ...
    def expect(self, pattern: str, timeout: float = 3) -> bool:
        end_time = time.time() + timeout
        while time.time() < end_time:
            line = self.readline()
            if line and pattern in line.decode(errors="ignore"):
                return True
            time.sleep(0.01)
        return False
...
def validate_dut_file(path: str) -> str:
    """Validate that the test file exists, is a Python script, and has setup/loop/teardown."""
    if not os.path.isfile(path):
        raise argparse.ArgumentTypeError(f"Test file '{path}' does not exist.")
    if not path.endswith(".py"):
        raise argparse.ArgumentTypeError("Test file must have a .py extension.")

    # Try to import the module from the given file path
    try:
        spec = importlib.util.spec_from_file_location("test_module", path)
        if spec is None or spec.loader is None:
            raise argparse.ArgumentTypeError(f"Could not load test file '{path}'.")
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
    except Exception as e:
        raise argparse.ArgumentTypeError(f"Failed to import test file '{path}': {e}")

    # Check required functions
    for fn in ("setup", "loop", "teardown"):
        if not hasattr(mod, fn):
            raise argparse.ArgumentTypeError(f"Test file '{path}' is missing required function: {fn}()")
    return path
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--test", "-t",
        metavar="FILE",
        type=validate_dut_file,
        help=(
            "Specify an AT command test file. "
            "The test file must implement setup(), loop(), and teardown(). "
            "If not specified, the tool will start in interactive REPL mode."
        )
    )
    parser.add_argument(
        '--port0-baudrate', '-p0b',
    ...
    parser.add_argument(
        '--log-tag', '-lt',
        action='store_true',
        help='Enable log prompt prefix for different AT ports. Default: False.'
    )
